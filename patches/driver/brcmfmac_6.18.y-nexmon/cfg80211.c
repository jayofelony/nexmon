...
#else
static int
brcmf_cfg80211_nexmon_set_channel(struct wiphy *wiphy, struct net_device *, struct cfg80211_chan_def *chandef) {
#endif
	struct brcmf_cfg80211_info *cfg = wiphy_to_cfg(wiphy);
	struct brcmf_if *ifp = netdev_priv(cfg_to_ndev(cfg));
	s32 err = 0;
	...
	/* This monitor-interface set_channel path is nexmon-specific and
	 * doesn't necessarily go through the same per-wiphy channel-list
	 * validation a managed-mode interface gets, so a caller can hand us
	 * a chandef for a band this chip never advertised (e.g. a 5GHz
	 * channel on a 2.4GHz-only part) - chandef_to_chanspec() below will
	 * happily encode whatever band it's given into a chanspec and this
	 * would go straight to the firmware's chanspec iovar. Reject that
	 * here instead: wiphy->bands[] only has entries for bands this chip
	 * actually supports, so this check is correct for every chip variant
	 * this driver runs on, not just 2.4GHz-only ones.
	 */
	if (!chandef->chan || !wiphy->bands[chandef->chan->band]) {
		brcmf_err("Set Channel rejected: unsupported band for this chip (chan=%p, band=%d)\n",
			  chandef->chan, chandef->chan ? chandef->chan->band : -1);
		return -EINVAL;
	}
